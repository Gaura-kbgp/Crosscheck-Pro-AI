...
def run():
    ...
    try:
        ...
        report_lines = []
        report_lines.append("==================================================")
        report_lines.append("REQUIRED FINAL METRICS")
        report_lines.append("==================================================")
        report_lines.append(f"Design items: {len(design_items)}")
        report_lines.append(f"Order items: {len(order_items)}")
        report_lines.append(f"Ack items: {len(ack_items)}")
        report_lines.append(f"Total Match Groups: {len(match_groups)}")
        ...
        report_lines.append("")
        report_lines.append("==================================================")
        report_lines.append("REQUIRED DETAILED CASES")
        report_lines.append("==================================================")
        ...
        # Find match groups touching target SKUs
        seen_mg_ids = set()
        for target in target_skus:
            for mg in match_groups:
                ...
                if any(target in s for s in skus) or any(target in s for s in canonical_skus) or target == mg.final_sku:
                    ...
                    d_qty = d_item.quantity if d_item else None
                    o_qty = o_item.quantity if o_item else None
                    a_qty = a_item.quantity if a_item else None
                    ...
                    intro_str = "; ".join([d.introduced_at or "N/A" for d in mg_discs]) if mg_discs else "N/A"
                    
                    report_lines.append(f"\n--- Match Group ID: {mg.id} ---")
                    report_lines.append(f"Design: {d_sku}")
                    report_lines.append(f"Order: {o_sku}")
                    report_lines.append(f"Ack: {a_sku}")
                    report_lines.append(f"Canonical SKU: {mg.final_sku}")
                    report_lines.append(f"Aggregate Qty: Design={d_qty}, Order={o_qty}, Ack={a_qty} (Final={mg.final_quantity})")
